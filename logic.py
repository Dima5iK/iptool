...
class NIC:
    def __init__(self, index:int, name:str, description:str, ip_addresses:list[IP], 
                 mac:str, status:bool, speed:int, received_bytes:int, sent_bytes:int):
        self.index:int = index
        self.name:str = name
        self.description = description
        self.ip_addresses:list[IP] = ip_addresses  # Список строк
        self.mac:str = mac
...
class PowerShellMonitor:
                ...
                try:
                    ...
                    for adapter in json_data:
                        ...
                        nic = NIC(
                            ...
                            description=adapter.get('Descrip'),
                            ip_addresses=ip_list,
                            mac=adapter.get('MacAddress'),
                            status=adapter.get('Status'),
                            speed=str(adapter.get('Speed', '0')),
                            received_bytes=int(adapter.get('ReceivedBytes', 0)),
                            sent_bytes=int(adapter.get('SentBytes', 0))
                        )
    ...
    def stop(self):
        """ Не актуально Останавливает мониторинг"""
        self.running = False
        if self.proc:
            self.proc.terminate()
    def get_IP_obj_from_str(ip_str_list:list[str]) ->list[IP] :
    ...
    def _parse_json_to_nics(self, json_data):
        """реобразует сырой JSON в список объектов NIC
"""
        nics = []
        if json_data:
            for adapter_data in json_data:
                nic = NIC(
